...
class Solver(object):
    ...
    def train(self):
        ...
        for epoch_i in trange(self.config.n_epochs, desc="Epoch", ncols=80):
            ...
            #     os.path.join(self.config.save_dir, f"model{epoch_i}.pt"),
            # )  # Save
            # Uncomment to save parameters at checkpoint

            # if epoch_i % 20 == 0:
            if epoch_i == self.config.n_epochs - 1:
                # if not os.path.exists(self.config.save_dir):
                #     os.makedirs(self.config.save_dir)
                # ckpt_path = str(self.config.save_dir) + f"/epoch-{epoch_i}.pt"
                ckpt_path = "./trained_model" + f"/epoch-{epoch_i}.pt"
    ...
    def evaluate(self, epoch_i, save_weights=False):
        ...
        for video_name, frame_features in tqdm(
            ...
            frame_features = frame_features.view(-1, 1024).to(self.config.device)
            ...
            scores_save_path = self.config.score_dir.joinpath(
            ...
            # if save_weights and (
            #     epoch_i + 1 == self.config.n_epochs or epoch_i + 1 == 0
            # ):
