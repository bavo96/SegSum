...
def inference(model, data_path, keys, eval_method):
    ...
    for video in keys:
        with h5py.File(data_path, "r") as hdf:
            ...
            frame_features = torch.Tensor(np.array(hdf[f"{video}/features"])).view(
                -1, 1024
            )
    ...
    print(
        f"CA-SUM model trained for split: {split_id} achieved an F-score: {np.mean(video_fscores):.2f}%",
        end="",
    )
...
if __name__ == "__main__":
    ...
    for split_id in range(5):
        # Model data
        # model_path = f"./pretrained_models/SumMe/split{split_id}/"
        ...
        f_score = inference(trained_model, dataset_path, test_keys, eval_metric)
